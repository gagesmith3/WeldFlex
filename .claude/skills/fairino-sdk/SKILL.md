...
| 15 | **`FRCNDEClient._robot_state_run_flag` latches `True`** | The recv loop `break`s on a socket error without clearing it and closes `_tcp_socket` without nulling it (`Robot.py:1826-1849`), so both stay truthy over a dead stream while `robot_state_pkg` holds its last values. Testing either serves a frozen cache as live data — require a live `_recv_thread` too (`RobotLink._cnde_streaming`). This is what made a probe read `CONNECTED` over dead XML-RPC; see `error-handling-and-connection.md` |
| 16 | **`CloseRPC()` always raises `AttributeError`** | `Robot.py:13737` reads `self.thread`, which is never assigned anywhere in `Robot.py`. Harmless only because `_teardown_raw` wraps it and the real cleanup happens first — never call it unwrapped, and keep the explicit `_udp_client.close()` after it, which `CloseRPC` never does |
