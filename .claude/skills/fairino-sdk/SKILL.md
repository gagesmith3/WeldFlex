...
| 9 | **`LuaUpload` code `-1` has two distinct sources** — read the errorStr before touching the Lua | Either the raw socket transfer on :20010 (five points in `__FileUpLoad`, nothing parsed yet) **or** the post-upload `LuaUpLoadUpdate` check refusing the file, which returns `(-1, errorStr)` — the errorStr carries the controller's actual reason. WeldFlex no longer calls `LuaUpload`: `robot_service._lua_upload` runs both steps itself and names the one that failed (`rpc`/`connect`/`send`/`reply`, or the post-upload errorStr). It also retries :20010's refusal of the first connect, which failed every upload from the Pi until 2026-09-23. See `program-and-file-management.md` |
...
| 15 | **`FRCNDEClient._robot_state_run_flag` latches `True`** | The recv loop `break`s on a socket error without clearing it and closes `_tcp_socket` without nulling it (`FRCNDEClient`'s receive loop in `Robot.py`), so both stay truthy over a dead stream while `robot_state_pkg` holds its last values. Testing either serves a frozen cache as live data — require a live `_recv_thread` too (`RobotLink._cnde_streaming`). This is what made a probe read `CONNECTED` over dead XML-RPC; see `error-handling-and-connection.md` |
| 16 | **`CloseRPC()` always raises `AttributeError`** | `Robot.py`'s `CloseRPC` reads `self.thread`, which is never assigned anywhere in `Robot.py`. Harmless only because `_teardown_raw` wraps it and the real cleanup happens first — never call it unwrapped, and keep the explicit `_udp_client.close()` after it, which `CloseRPC` never does |
