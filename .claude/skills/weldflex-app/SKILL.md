...
(gotchas 7 and 9 below): there is no arming-confirmation step between loading a
live job and starting it, and `pause_points` is a dead field.

## App shape

- **Page routes**: `/operator/<name>` (renders a full page extending
  `base.html`) and `/manager/<tab>` (`part-designer`/`settings`/`reports`).
  Bare `/manager` redirects to `/manager/part-designer` — see
  `references/routes-and-templates.md` for the duplicate-route bug behind that.
...
| 6 | `/operator/calibrate` + 6 `/ui/calibrate/*` routes are **not implemented** and, since 2026-09-24, no longer linked from `calibration.html` (the menu is Jog, Force Sensor and Points; `/operator/tcp-calibrate` still exists but is unlinked too) — next planned feature | `references/state-and-session.md`, `fairino-sdk`'s `coordinate-calibration.md` |
| 7 | **A run's mode is two switches: Live/Dry per run, DI check per recipe.** Since 2026-08-03 `WeldFlex.lua`'s cycle loop calls `weld.lua` per stud (`NewDofile("/fruser/weld.lua", 1, 1)`). Since 2026-09-14 `lua_builder.RunMode` resolves the mode once and both callers publish `WELD_ARMED` and `WELD_DI_CHECK` verbatim through `--{{RUN_MODE}}`; no Lua file derives them. `arm_mode` has **no default** at any layer: `/ui/job/load` and `/ui/single-shot/fire` return an error without one, and `JobManager.load()` requires it. `di_check` is a recipe field (default on); off skips the DI0 wait and both DI1 checks, **live runs included** (owner decision; the welder profile and Liberty guards are gone). What still does not exist is a confirmation at the moment Run is pressed on a loaded live job. The job panel shows LIVE/DRY and DI OFF | `backend/lua_builder.py`, `programs/weld.lua`, `docs/ARCHITECTURE.md` |
| 8 | **`JobManager` runs more than customer parts.** `load()`/`_launch()` take a `kind` (`"part"` default, `"single_shot"`, anything else raises) that picks `build_weldflex_lua` vs. `build_single_shot_lua`. Everything downstream (monitor thread, `CycleTracker`, `partials/current_job.html`) is shared and kind-unaware. Adding a third kind means extending `JOB_KINDS` and this branch, not forking a new manager | `backend/job_manager.py`, `references/routes-and-templates.md`'s `single-shot` section |
| 9 | **`pause_points` is a dead field** — written as `[]` on every recipe (two sites: `_single_shot_recipe()`/`ui_recipes_save()`), read by nothing. `lua_builder._stud_rows` consumes only `x`/`y`. Per-stud operator waits do **not** exist; `gate_mode` is per-*cycle* and is a different feature | `backend/app.py`, `backend/lua_builder.py` |
...
| 14 | **The job manager reads `get_universal_state()`** — `_monitor_body`'s `snap = self._robot.get_universal_state()` observes through the service's consolidated cache, so cycle counting takes its line from the 8083 feed and rides through the force-op outage with everything else. **This inverted in `f41dd0a`**: it previously read `self._robot.snapshot()` directly and stalled, and older notes (including earlier revisions of this row) say so. Consequence: changing `get_universal_state()` now *does* affect cycle counting — they are no longer separate paths | `backend/job_manager.py` |
| 15 | **Routes read caches; they never call the robot.** `robot.get_universal_state()` and `robot.ft_read()` issue no RPC at all — `ft_read` lost its XML-RPC fallback, so two stale caches render "no reading" rather than reaching for the wire. This is what makes `/ui/ft/stream` (SSE, 10 Hz, 4 concurrent slots, 30 s life) affordable; put an RPC back behind `/ui/ft/reading` and you have created 10 round trips a second per viewer. The only host-initiated traffic is the heartbeat, the telemetry sampler, and operator commands | `docs/ROBOT_TELEMETRY.md` "Host-side discipline", `references/routes-and-templates.md` |
| 16 | **Telemetry reads are bounded and abortable, on purpose.** The link has one SDK worker, so an operator pressing Stop queues behind whatever read is in flight. `weld_probe` therefore issues one `rpc_timeout`-capped call per sysvar slot instead of one batched dispatch, and aborts the whole sample on the first failure; `RobotLink.call()` drops a queued call whose caller already timed out. Don't "optimise" these back into a single long batch — the batch was the problem | `backend/robot_service.py`, `backend/robot_link.py` |
| 17 | **A part's stored stud X/Y are not robot offsets.** They are measured inward from the part's `origin_corner`. Only front-left, the taught `zerozero`, matches the robot's offsets. Any code that moves to a stud must resolve it through `part_origin.resolve_studs()`/`resolve_point()` first, as `lua_builder`, `JobManager.load` and `/ui/parts/goto` do. Passing `recipe["studs"]` straight into a `PointsOffsetEnable` is correct only for front-left parts, which is why it would pass every existing test. Every other corner needs its taught point (`zerozero_fr`/`_bl`/`_br`), read through `part_origin.read_corner_ref()` — `resolve_studs` refuses one without its `CornerRef` | `backend/part_origin.py`, `references/state-and-session.md` |
