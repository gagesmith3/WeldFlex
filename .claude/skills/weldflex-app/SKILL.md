...
| 17 | **A part's stored stud X/Y are not robot offsets.** They are measured inward from the part's `origin_corner`. Only front-left, the taught `zerozero`, matches the robot's offsets. Any code that moves to a stud must resolve it through `part_origin.resolve_studs()`/`resolve_point()` first, as `lua_builder`, `JobManager.load` and `/ui/parts/goto` do. Passing `recipe["studs"]` straight into a `PointsOffsetEnable` is correct only for front-left parts, which is why it would pass every existing test. The other corners are computed from `WELDFLEX_BED_X_MM`/`_Y_MM`, not taught | `backend/part_origin.py`, `references/state-and-session.md` |
