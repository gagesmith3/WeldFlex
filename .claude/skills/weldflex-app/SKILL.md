...
| 9 | **`pause_points` is a dead field** — written as `[]` on every recipe (two sites in `_recipes_enrich`/`ui_recipes_save`), read by nothing. `lua_builder._stud_rows` consumes only `x`/`y`. Per-stud operator waits do **not** exist; `gate_mode` is per-*cycle* and is a different feature | `backend/app.py`, `backend/lua_builder.py` |
