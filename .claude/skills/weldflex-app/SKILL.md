...
| 7 | **A run welds for real. A dry path exists; an arming confirmation does not.** Since 2026-08-03 `WeldFlex.lua`'s cycle loop calls `weld.lua` per stud (`NewDofile("/fruser/weld.lua", 1, 1)`). Since `6722285` (2026-09-02) the arc is gated on `WELD_ARMED`: `weld.lua:535` skips the pulse when it is not `1`, and `WeldFlex.lua:47-49` sets it from the job's arm mode, so a dry job runs the full search/press/hold/retract/feed sequence without firing. What does **not** exist is an explicit arm/disarm confirmation — loading and starting a live job is the same two clicks as a dry one, so "dry" is a property of the job that was loaded, not a state the operator confirms at the moment they press Run. Don't describe a run as safely dry-runnable without saying which of those two you mean | `programs/WeldFlex.lua`, `programs/weld.lua`, `docs/ARCHITECTURE.md` |
